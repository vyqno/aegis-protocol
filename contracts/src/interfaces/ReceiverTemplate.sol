...
///      NOTE: This contract uses Ownable (not Ownable2Step). For production vaults,
///      override transferOwnership() at the inheriting contract level (see VULN-002).
///      NOTE: setForwarderAddress(address(0)) disables ALL CRE report validation.
///      Override setForwarderAddress() in production contracts to prevent this (see VULN-001).
abstract contract ReceiverTemplate is IReceiver, IERC165, Ownable {
  ...
  /// @dev VULN-004: Validates metadata length >= 62 bytes before assembly read
  function _decodeMetadata(
    bytes memory metadata
  ) internal pure returns (bytes32 workflowId, bytes10 workflowName, address workflowOwner) {
    // Metadata must be at least 62 bytes: 32 (workflowId) + 10 (workflowName) + 20 (workflowOwner)
